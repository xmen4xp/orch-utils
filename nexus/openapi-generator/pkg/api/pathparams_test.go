...
func TestResolveUriParams(t *testing.T) {
	...
	tests := []struct {
		...
		// wantTokens is the expected [_][1] of the resolved [][]string.
		wantTokens []string
	}{
		{
			name:       "nil PathParams passes through canonical tokens",
		...
		{
			name:       "mixed alias and unmapped canonical token preserved",
			uri:        "/v1/organizations/{org}/projects/{projects.Project}",
