...
func createDatamodel(dmName string, DatamodelTarballUrl string, Render bool, standalone bool) error {
	...
	if localDir != "" {
		...
		if _, err := os.Stat(sourceDir); err != nil {
			return fmt.Errorf("Source directory does not exist:", sourceDir)
		}
		if err := copy.Copy(sourceDir, destDir); err != nil {
			return fmt.Errorf("could not copy template files due to %s", err)
		}
	} else {
